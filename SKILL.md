---
name: tomato-writer
description: 番茄小说（fanqienovel.com）自动化发布 skill。当用户需要操作番茄小说创作中心时优先使用，包括：发布章节、修改章节标题或正文、删除章节、查看章节列表、章节详情、查看/新建/重命名/删除分卷、查看作品列表、保存登录 Cookie，以及 AI 辅助写章节、生成大纲、管理故事设定集等。通过 Playwright 浏览器自动化实现，Cookie session 持久化免重复登录。只要用户提到番茄小说、发章节、改正文、分卷管理、写小说、生成大纲、故事设定等创作相关操作，就应优先调用本 skill。
---
...
**0. 环境检查（首次使用必做）**
- 确认 `node_modules/` 存在（否则先 `npm install`）
- 确认 `~/.tomato-writer-session.json` 存在（否则走步骤 1）
- 快速验证 Cookie 是否有效：`node scripts/tomato.js list-books`
  - 返回 `ok:true` → 环境就绪，继续
  - 返回「Cookie 已过期」或「未找到 session」→ 走步骤 1
...
**2. 初始化故事 / 生成大纲**
- 用户提供世界观脑洞 → 读取 `story/主编.md`，按主编工作规范生成大纲和节拍器
- 填写「世界观核心」「故事主线大纲」「章节节拍器」
- 有新角色/地点/规则 → 同步写入 `story/设定集库.md`
- 如果 `story/主编.md` 不存在，先创建该文件（世界观空白模板）再填写

**3. 撰写章节正文**
- 写作前必须加载：
  - `story/主编.md` → 找到对应章节的节拍器
  - `story/设定集库.md` → 确认出场人物、地点、规则不出错
- 按节拍器写正文（不少于 1000 字）
- 写完后检查「设定更新」字段，有新增内容 → 追加到 `story/设定集库.md`
- 将正文写入 `/tmp/tomato-content.txt`，再发布：`publish-chapter --content-file /tmp/tomato-content.txt`

**4. 发布新章节（无需 AI 写作，直接发布）**
- 若用户已知 bookId → 直接 `publish-chapter`
- 若不知道 bookId → 先 `list-books` 获取，再 `publish-chapter`
- 正文内容先用 `write_file` 工具写入 `/tmp/tomato-content.txt`，再用 `--content-file /tmp/tomato-content.txt` 传入
- ⚠️ `--title` 只写标题本身，**不含「第X章」**——例如 `--title "冰川开裂"` 而非 `--title "第1章 冰川开裂"`

**5. 修改章节标题 / 正文**
- 先 `list-chapters --book-id <id>` 获取章节列表（含 chapterId），**不要猜测 chapterId**
...
>
> ⚠️ **`--title` 只填标题内容，不含「第X章」前缀**。`--chapter-num` 已填序号，平台展示时自动组合为「第X章 标题」。若 title 里再写「第1章 冰川开裂」，章节列表会显示为「第1章 第1章 冰川开裂」。
...
每次发布 / 修改章节，脚本自动按顺序处理以下弹窗：

1. **填写表单** → 章节序号 + 标题 + 正文
2. **点"下一步"** → 触发分卷向导侧边栏（出现 2～3 个"下一步"按钮）
3. **分卷向导** → 点"下一步"×多次跳过
4. **"我知道了"弹窗** → 自动点击跳过（平台新增提示）
5. **分卷向导"提交"** → 自动点击（提交分卷配置，不是最终发布）
6. **写作检测弹窗**（若出现）→ 自动点"忽略全部"跳过
7. **风险检测弹窗** → 自动点"取消"跳过
8. **发布设置弹窗** → 自动选「是否使用AI = **是**」→ 点"确认发布"

发布成功后章节进入**审核中**状态，审核通过后正式上线。

> ⚠️ **若发布命令返回 `ok:false` 或弹窗处理卡住**，请查看 stderr 输出的 `[flow stepN]` 日志，对照 [`references/publish-troubleshooting.md`](references/publish-troubleshooting.md) 排查。平台偶尔会调整弹窗顺序，troubleshooting.md 记录了已知的所有变化。
